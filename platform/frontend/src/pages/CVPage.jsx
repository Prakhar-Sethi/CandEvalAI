...
import {
  FileText, Upload, X, Loader2, CheckCircle2, AlertCircle, ChevronRight,
} from 'lucide-react'
...
export default function CVPage() {
  ...
  const handleSubmit = (e) => {
    ...
    localStorage.setItem('plt_candidate_id', form.candidate_id)
    localStorage.setItem('plt_candidate_name', form.name)
    localStorage.setItem('plt_candidate_email', form.email)
    localStorage.setItem('plt_cv_data', JSON.stringify(cvData || {}))
    localStorage.setItem('plt_skills', JSON.stringify(cvData?.skills || []))
    localStorage.setItem('plt_step', '2')
    navigate('/written-test')
  }
      ...
      <div className="flex-1 flex items-center justify-center px-6 py-10">
        <div className="w-full max-w-2xl">
          <div className="text-center mb-8">
            <h1 className="text-3xl font-bold text-slate-900 dark:text-slate-100">CV Scan & Registration</h1>
            <p className="text-slate-500 mt-2">Upload your resume — we'll extract skills automatically</p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <div className="bg-white dark:bg-white/5 shadow-sm dark:shadow-none backdrop-blur-xl border border-slate-200 dark:border-white/10 rounded-2xl p-6">
              <p className="text-sm font-medium text-slate-700 dark:text-slate-300 mb-4 flex items-center gap-2">
                ...
                Upload Resume <span className="text-slate-400 font-normal">(optional — auto-fills skills)</span>
              </p>
              ...
              {!resumeFile ? (
                <div
                  ...
                  className={`border-2 border-dashed rounded-xl p-8 text-center cursor-pointer transition-all ${
                    dragOver ? 'border-sky-400 bg-sky-50 dark:bg-sky-500/10' : 'border-slate-200 dark:border-white/15 hover:border-sky-300 dark:hover:border-sky-400/40 hover:bg-slate-50 dark:hover:bg-white/[0.03]'
                  }`}
                >
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    {resumeState === 'done' && (
                      <p className="text-emerald-600 dark:text-emerald-400 text-xs flex items-center gap-1 mt-0.5">
                        ...
                        {cvData?.skills?.length ? `${cvData.skills.length} skills detected` : 'Parsed — select skills manually on next step'}
                      </p>
                    )}
                    {resumeState === 'error' && <p className="text-red-500 dark:text-red-400 text-xs flex items-center gap-1 mt-0.5"><AlertCircle className="w-3 h-3" /> {resumeError}</p>}
                  </div>
              ...
              {cvData?.skills?.length > 0 && (
                <div className="mt-4 flex flex-wrap gap-2">
                  {cvData.skills.map((s) => (
                    <span key={s} className="px-3 py-1 rounded-lg bg-sky-50 dark:bg-sky-500/15 border border-sky-200 dark:border-sky-500/25 text-sky-700 dark:text-sky-300 text-xs font-medium">{s}</span>
                  ))}
                </div>
              )}
            </div>
            ...
            <div className="bg-white dark:bg-white/5 shadow-sm dark:shadow-none backdrop-blur-xl border border-slate-200 dark:border-white/10 rounded-2xl p-6 space-y-4">
              <p className="text-sm font-medium text-slate-700 dark:text-slate-300">Candidate Details</p>
              ...
              <div>
                <label className="block text-xs text-slate-500 mb-1.5">Candidate ID</label>
                <input type="text" value={form.candidate_id} onChange={(e) => setForm({ ...form, candidate_id: e.target.value })} placeholder="e.g. CAND-2024-001"
                  className="w-full bg-white dark:bg-white/5 border border-slate-200 dark:border-white/10 rounded-xl px-4 py-2.5 text-slate-900 dark:text-slate-200 placeholder-slate-400 dark:placeholder-slate-500 focus:outline-none focus:border-indigo-500 dark:focus:border-indigo-500/50 focus:ring-2 focus:ring-indigo-500/20 transition text-sm" />
              </div>
            </div>
            ...
            <button type="submit" disabled={resumeState === 'parsing'}
              ...
              Proceed to Written Test <ChevronRight className="w-5 h-5" />
            </button>
          </form>
        </div>
      </div>
